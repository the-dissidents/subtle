...
macro_rules! send_error {
    ($channel:expr, $what:expr) => {
        $channel.send(MediaEvent::RuntimeError {
...
fn send_nomedia(channel: &Channel<MediaEvent>) {
    channel.send(MediaEvent::NoMedia {  }).expect("Error sending event");
}
...
pub fn media_status(
    ...
    send(&channel, MediaEvent::MediaStatus { 
...
pub fn get_intensities(
    ...
    while let Ok(Some(resampled_frame)) = playback.next_audio_frame() {
        ...
        if playback.audio().unwrap().position() >= until { break; }
    }

    return send(&channel, MediaEvent::IntensityList { 
